...
*
* This include requires the two defines {\ref KALMAN_MEASUREMENT_NAME} and {\ref
KALMAN_NUM_MEASUREMENTS}
* to be set to the base name of the Kalman Filter measurement and to the number
of measured outputs,
* as well as all defines from \ref {kalman_factory_filter.h}
*
* It then will instantiate the buffers required for H, R, z and all helper
matrices and vectors,
* as well as the structure for the Kalman filter measurement and the
initialization method.
*
* Suppose a Kalman filter named "direction" for which a measurement named
"gyroscope" shall be crated
* which has three measured outputs. You would start by defining the required
macros
*
* \code{.c}
...
* \endcode
*
* At this point, the structure \c kalman_filter_direction_measurement_gyroscope
will be created (statically) along with
* all the required buffers (i.e. \c
kalman_filter_direction_measurement_gyroscope_H_buffer, etc.) and the matrices
* will be initialized and set with the correct dimensions.
*
* In addition, a parameterless static initialization function \code
{kalman_filter_direction_measurement_gyroscope_init()} will
* be created which you will need to call manually in order to set up the
measurement.
*
...
* \endcode
*
* In order to force creation of separate auxiliary buffers (thus preventing
buffer reuse), MEASUREMENT_FORCE_NEW_BUFFERS can be defined
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                                              \
	"** Instantiating Kalman filter \"" STRINGIFY(KALMAN_NAME) "\" measurement \"" STRINGIFY( \
		KALMAN_MEASUREMENT_NAME) "\" with " STRINGIFY(KALMAN_NUM_MEASUREMENTS) " measured outputs")
#endif

#if MEASUREMENT_FORCE_NEW_BUFFERS
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"MEASUREMENT_FORCE_NEW_BUFFERS was set. Forcing separate auxiliary buffers.")
#endif
#endif
...
#define __KALMAN_maux_ROWS                                                     \
	((KALMAN_NUM_STATES > KALMAN_NUM_MEASUREMENTS) ? KALMAN_NUM_STATES         \
												   : KALMAN_NUM_MEASUREMENTS)
#define __KALMAN_maux_COLS 1
#define __USE_BUFFER_AUX                                                       \
	((__KALMAN_maux_ROWS * __KALMAN_maux_COLS) <= __KALMAN_aux_size)
...
#define KALMAN_MEASUREMENT_BASENAME_HELPER2(basename, measname)                \
	basename##_measurement_##measname
#define KALMAN_MEASUREMENT_BASENAME_HELPER(basename, measname)                 \
	KALMAN_MEASUREMENT_BASENAME_HELPER2(basename, measname)
#define KALMAN_MEASUREMENT_BASENAME                                            \
	KALMAN_MEASUREMENT_BASENAME_HELPER(KALMAN_FILTER_BASENAME,                 \
									   KALMAN_MEASUREMENT_NAME)

#define KALMAN_MEASUREMENT_STRUCT_HELPER2(name) #name
#define KALMAN_MEASUREMENT_STRUCT_HELPER(name)                                 \
	KALMAN_MEASUREMENT_STRUCT_HELPER2(name)
#define KALMAN_MEASUREMENT_STRUCT_NAME                                         \
	KALMAN_MEASUREMENT_STRUCT_HELPER(KALMAN_FILTER_BASENAME)

#define KALMAN_MEASUREMENT_FUNCTION_HELPER2(basename, measname, element)       \
	basename##_measurement_##measname##_##element
#define KALMAN_MEASUREMENT_FUNCTION_HELPER(basename, measname, element)        \
	KALMAN_MEASUREMENT_FUNCTION_HELPER2(basename, measname, element)
#define KALMAN_MEASUREMENT_FUNCTION_NAME(action)                               \
	KALMAN_MEASUREMENT_FUNCTION_HELPER(KALMAN_FILTER_BASENAME,                 \
									   KALMAN_MEASUREMENT_NAME, action)

#define KALMAN_MEASUREMENT_BUFFER_HELPER2(basename, measname, element)         \
	basename##_measurement_##measname##_##element##_buffer
#define KALMAN_MEASUREMENT_BUFFER_HELPER(basename, measname, element)          \
	KALMAN_MEASUREMENT_BUFFER_HELPER2(basename, measname, element)
#define KALMAN_MEASUREMENT_BUFFER_NAME(element)                                \
	KALMAN_MEASUREMENT_BUFFER_HELPER(KALMAN_FILTER_BASENAME,                   \
									 KALMAN_MEASUREMENT_NAME, element)
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman measurement H buffer: " STRINGIFY(__KALMAN_BUFFER_H))
#endif
static matrix_data_t __KALMAN_BUFFER_H[__KALMAN_H_ROWS * __KALMAN_H_COLS];

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman measurement R buffer: " STRINGIFY(__KALMAN_BUFFER_R))
#endif
static matrix_data_t __KALMAN_BUFFER_R[__KALMAN_R_ROWS * __KALMAN_R_COLS];

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman measurement z buffer: " STRINGIFY(__KALMAN_BUFFER_z))
#endif
static matrix_data_t __KALMAN_BUFFER_z[__KALMAN_z_ROWS * __KALMAN_z_COLS];

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman measurement K buffer: " STRINGIFY(__KALMAN_BUFFER_K))
#endif
static matrix_data_t __KALMAN_BUFFER_K[__KALMAN_K_ROWS * __KALMAN_K_COLS];

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman measurement S buffer: " STRINGIFY(__KALMAN_BUFFER_S))
#endif
static matrix_data_t __KALMAN_BUFFER_S[__KALMAN_S_ROWS * __KALMAN_S_COLS];

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman measurement y buffer: " STRINGIFY(__KALMAN_BUFFER_y))
#endif
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Re-using Kalman aux buffer for measurement aux buffer: " STRINGIFY(       \
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message("Creating Kalman measurement aux buffer: " STRINGIFY(          \
	__KALMAN_BUFFER_maux))
#endif
...
#if __KALMAN_Sinv_size <= __KALMAN_tempPBQ_size &&                             \
	!MEASUREMENT_FORCE_NEW_BUFFERS

#define __KALMAN_BUFFER_Sinv __KALMAN_BUFFER_tempPBQ
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                                                \
	"Re-using Kalman filter temporary P/BQ buffer for temporary S-inverted buffer: " STRINGIFY( \
		__KALMAN_BUFFER_Sinv))
#endif
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman measurement temporary S-inverted buffer: " STRINGIFY(     \
		__KALMAN_BUFFER_Sinv))
#endif
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman measurement temporary HxP buffer: " STRINGIFY(            \
		__KALMAN_BUFFER_tempHP))
#endif
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                                       \
	"Re-using Kalman measurement temporary HxP for temporary PxH' buffer: " STRINGIFY( \
		__KALMAN_BUFFER_tempPHt))
#endif
...
#pragma message(                                                               \
	"Creating Kalman measurement temporary PxH' buffer: " STRINGIFY(           \
		__KALMAN_BUFFER_tempPHt)) // TODO: reuse HxP buffer!
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                                                          \
	"Re-using Kalman filter temporary P/BQ buffer for measurement temporary Kx(HxP)  buffer: " STRINGIFY( \
		__KALMAN_BUFFER_tempKHP))
#endif
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman measurement temporary Kx(HxP) buffer: " STRINGIFY(        \
		__KALMAN_BUFFER_tempKHP))
#endif
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message("Creating Kalman measurement structure: " STRINGIFY(           \
	KALMAN_MEASUREMENT_BASENAME))
#endif
static kalman_measurement_t KALMAN_MEASUREMENT_BASENAME;

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman measurement initialization function: " STRINGIFY(         \
		KALMAN_MEASUREMENT_FUNCTION_NAME(init())))
#endif
...
static kalman_measurement_t *KALMAN_MEASUREMENT_FUNCTION_NAME(init)()
{
	...
	kalman_measurement_initialize(
		&KALMAN_MEASUREMENT_BASENAME, KALMAN_NUM_STATES,
		KALMAN_NUM_MEASUREMENTS, __KALMAN_BUFFER_H, __KALMAN_BUFFER_z,
		__KALMAN_BUFFER_R, __KALMAN_BUFFER_y, __KALMAN_BUFFER_S,
		__KALMAN_BUFFER_K, __KALMAN_BUFFER_maux, __KALMAN_BUFFER_Sinv,
		__KALMAN_BUFFER_tempHP, __KALMAN_BUFFER_tempPHt,
		__KALMAN_BUFFER_tempKHP);
...
// TODO: instead of cleaning up the temporary buffers here, clean them up in
// kalman_factory_cleanup.h. This way, the largest buffers can be reused in
