...
*
* This include requires the three defines {\ref KALMAN_NAME}, {\ref
KALMAN_NUM_STATES} and
* {\ref KALMAN_NUM_INPUTS} to be set to the base name of the Kalman Filter and
to the number
* of states and inputs respectively.
*
* It then will instantiate the buffers required for A, P, x as well as B, Q and
u if the number
* of inputs is greater than zero, as well as the structure for the Kalman filter
and the
* initialization method.
*
* Suppose the Kalman filter shall be named "acceleration", has three states and
zero inputs.
...
* \endcode
*
* At this point, the structure \c kalman_filter_acceleration will be created
(statically) along with
* all the required buffers (i.e. \c kalman_filter_acceleration_A_buffer, etc.)
and the matrices
* will be initialized and set with the correct dimensions.
*
* In addition, a parameterless static initialization function \c
{kalman_filter_acceleration_init()} will
* be created which you will need to call manually in order to set up the filter.
*
* To clean up the defined macros (e.g. in order to be able to create another
named Kalman filter),
...
#define __KALMAN_aux_ROWS                                                      \
	((KALMAN_NUM_STATES > KALMAN_NUM_INPUTS) ? KALMAN_NUM_STATES               \
											 : KALMAN_NUM_INPUTS)
...
#pragma message(                                                                     \
	"** Instantiating Kalman filter \"" STRINGIFY(KALMAN_NAME) "\" with " STRINGIFY( \
		KALMAN_NUM_STATES) " states and " STRINGIFY(KALMAN_NUM_INPUTS) " inputs")
#endif
...
#define KALMAN_BUFFER_HELPER(basename, element)                                \
	KALMAN_BUFFER_HELPER2(basename, element)
#define KALMAN_BUFFER_NAME(element)                                            \
	KALMAN_BUFFER_HELPER(KALMAN_BASENAME_HELPER(KALMAN_FILTER_BASENAME),       \
						 element)

#define KALMAN_FUNCTION_HELPER2(basename, name) basename##name
#define KALMAN_FUNCTION_HELPER(basename, name)                                 \
	KALMAN_FUNCTION_HELPER2(basename, name)
#define KALMAN_FUNCTION_NAME(name)                                             \
	KALMAN_FUNCTION_HELPER(KALMAN_BASENAME_HELPER(KALMAN_FILTER_BASENAME), name)
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman filter A buffer: " STRINGIFY(__KALMAN_BUFFER_A))
#endif
static matrix_data_t __KALMAN_BUFFER_A[__KALMAN_A_ROWS * __KALMAN_A_COLS];

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman filter P buffer: " STRINGIFY(__KALMAN_BUFFER_P))
#endif
static matrix_data_t __KALMAN_BUFFER_P[__KALMAN_P_ROWS * __KALMAN_P_COLS];

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman filter x buffer: " STRINGIFY(__KALMAN_BUFFER_x))
#endif
static matrix_data_t __KALMAN_BUFFER_x[__KALMAN_x_ROWS * __KALMAN_x_COLS];

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman filter Q buffer: " STRINGIFY(__KALMAN_BUFFER_Q))
#endif
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman filter B buffer: " STRINGIFY(__KALMAN_BUFFER_B))
#endif
static matrix_data_t __KALMAN_BUFFER_B[__KALMAN_B_ROWS * __KALMAN_B_COLS];

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman filter u buffer: " STRINGIFY(__KALMAN_BUFFER_u))
#endif
...
#define __KALMAN_tempPBQ_size                                                  \
	((__KALMAN_tempP_size > __KALMAN_tempBQ_size) ? __KALMAN_tempP_size        \
												  : __KALMAN_tempBQ_size)

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman filter aux buffer: " STRINGIFY(__KALMAN_BUFFER_aux))
#endif
static matrix_data_t __KALMAN_BUFFER_aux[__KALMAN_aux_size];

#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message("Creating Kalman filter temporary P/BQ buffer: " STRINGIFY(    \
	__KALMAN_BUFFER_tempPBQ))
#endif
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message(                                                               \
	"Creating Kalman filter structure: " STRINGIFY(KALMAN_STRUCT_NAME))
#endif
...
#ifdef ENABLE_KALMAN_LIB_PRAGMA
#pragma message("Creating Kalman filter initialization function: " STRINGIFY(  \
	KALMAN_FUNCTION_NAME(init())))
#endif
...
static kalman_t *KALMAN_FUNCTION_NAME(init)()
{
	...
	kalman_filter_initialize(
		&KALMAN_STRUCT_NAME, KALMAN_NUM_STATES, KALMAN_NUM_INPUTS,
		__KALMAN_BUFFER_A, __KALMAN_BUFFER_x, __KALMAN_BUFFER_B,
		__KALMAN_BUFFER_u, __KALMAN_BUFFER_P, __KALMAN_BUFFER_Q,
		__KALMAN_BUFFER_aux, __KALMAN_BUFFER_aux, __KALMAN_BUFFER_tempPBQ,
		__KALMAN_BUFFER_tempPBQ);
